...
import { RtpCapabilities, DtlsParameters, SctpCapabilities, ConsumerParams, WebRtcTransportParams } from '@/chat-service/src/signaling.types'; // Adjust path as needed
...
export class WebRTCClient {
  ...
  constructor() {
    if (!this.socket) {
      ...
      throw new Error("Socket not initialized for WebRTCClient. Ensure getSocket() provides a connected socket.");
    }
  }
  ...
  public async loadDevice(currentRoomId: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      emitGetRouterRtpCapabilities(this.roomId!, (routerRtpCapabilities) => {
        if ('error' in routerRtpCapabilities) {
          console.error('Failed to get Router RTP Capabilities:', routerRtpCapabilities.error);
          return reject(new Error(`Failed to get Router RTP Capabilities: ${routerRtpCapabilities.error}`));
        }
        try {
          this.device = new mediasoupClient.Device();
          this.device.load({ routerRtpCapabilities: routerRtpCapabilities as RtpCapabilities })
            .then(() => {
  ...
  public async createSendTransport(): Promise<mediasoupClient.types.Transport> {
    ...
    return new Promise((resolve, reject) => {
      emitCreateWebRtcTransport(
        { roomId: this.roomId!, producing: true, consuming: false, sctpCapabilities: this.device?.sctpCapabilities },
        (transportParams) => {
          if ('error' in transportParams) {
            console.error('Error creating send transport on server:', transportParams.error);
          ...
          this.sendTransport.on('connect', async ({ dtlsParameters }, callback, errback) => {
            ...
            emitConnectWebRtcTransport(
              { roomId: this.roomId!, transportId: this.sendTransport!.id, dtlsParameters },
              (response) => {
                if (response.error) {
                  console.error('Error connecting send transport on server:', response.error);
          ...
          this.sendTransport.on('produce', async ({ kind, rtpParameters, appData }, callback, errback) => {
            console.log('Send transport produce event:', { kind, appData });
            emitProduce(
              { roomId: this.roomId!, transportId: this.sendTransport!.id, kind, rtpParameters, appData },
              (response) => {
                if (response.error || !response.producerId) {
                  console.error(`Error producing ${kind} on server:`, response.error);
                  errback(new Error(response.error || 'Producer ID not returned'));
                } else {
                  console.log(`${kind} produced successfully on server, producerId:`, response.producerId);
  ...
  public async createRecvTransport(): Promise<mediasoupClient.types.Transport> {
    ...
    return new Promise((resolve, reject) => {
      emitCreateWebRtcTransport(
        { roomId: this.roomId!, producing: false, consuming: true, sctpCapabilities: this.device?.sctpCapabilities },
        (transportParams) => {
          if ('error' in transportParams) {
            console.error('Error creating recv transport on server:', transportParams.error);
            return reject(new Error(transportParams.error));
          }
          ...
          this.recvTransport.on('connect', ({ dtlsParameters }, callback, errback) => {
            ...
            emitConnectWebRtcTransport(
              { roomId: this.roomId!, transportId: this.recvTransport!.id, dtlsParameters },
              (response) => {
                if (response.error) {
                  console.error('Error connecting recv transport on server:', response.error);
                  errback(new Error(response.error));
                } else {
  ...
  public async produce(track: MediaStreamTrack, appData?: any): Promise<mediasoupClient.types.Producer> {
    ...
    producer.on('trackended', () => {
      console.log(`${track.kind} track ended (e.g., user stopped sharing)`);
  ...
  public async consume(producerId: string, kind: 'audio' | 'video', rtpCapabilities: RtpCapabilities, appData?: any): Promise<ConsumerInfo | null> {
    if (!this.recvTransport) {
        console.error('Receive transport not available for consuming');
        throw new Error('Receive transport not created');
    }
    if (!this.device?.canProduce('video') && kind === 'video') { // Check based on kind
        console.warn(`Device cannot produce ${kind}, so cannot consume for it with current RTP capabilities.`);
        ...
        emitConsume(
            ...
            async (consumerParams) => {
                if ('error' in consumerParams || !consumerParams.id) {
                    console.error(`Error consuming ${kind} for producer ${producerId} on server:`, ('error' in consumerParams ? consumerParams.error : 'No params'));
                    return reject(new Error('error' in consumerParams ? consumerParams.error : 'Failed to get consumer parameters'));
                }
                ...
                try {
                    const consumer = await this.recvTransport!.consume({
                        ...
                        appData: { ...typedParams.appData, consumingUserId: 'self' } // Or actual user ID from context
                    });
                    ...
                    console.error('Error creating mediasoup consumer:', error);
                    reject(error);
                }
            }
        );
  ...
  public async produceScreenShare(payload: { track: MediaStreamTrack; appData?: any }): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      this.socket.emit('startScreenShare', 
        { 
          roomId: this.roomId!, 
        ...
        (response: { producerId?: string; error?: string }) => {
          if (response.error || !response.producerId) {
            console.error('Error producing screen share on server:', response.error);
            reject(new Error(response.error || 'Screen share producer ID not returned'));
          } else {
            console.log('Screen share produced successfully on server, producerId:', response.producerId);
  ...
  public async closeScreenShareProducer(producerId: string, currentRoomId: string): Promise<void> {
    if (!producerId || !currentRoomId) {
        console.warn('Missing producerId or roomId for closing screen share producer.');
        return Promise.resolve();
    }
    console.log(`Requesting server to close screen share producer: ${producerId} in room ${currentRoomId}`);
    ...
    const screenProducer = Array.from(this.producers.values()).find(p => p.id === producerId && p.appData.type === 'screen');
    if (screenProducer) {
    ...
    return new Promise<void>((resolve, reject) => {
        this.socket.emit('stopScreenShare', { roomId: currentRoomId, producerId }, (response: { error?: string }) => {
            if (response.error) {
                console.error(`Error stopping screen share producer ${producerId} on server:`, response.error);
  ...
  public replaceTrack(kind: 'audio' | 'video', newTrack: MediaStreamTrack | null): Promise<void> {
    const producer = this.producers.get(kind);
    if (!producer) {
        console.warn(`No producer found for kind ${kind} to replace track.`);
    ...
    if (!newTrack) { // If newTrack is null, it means we want to stop sending this kind of track
        console.log(`Stopping ${kind} producer by replacing track with null.`);
        this.closeProducer(kind); // Or producer.pause() and signal mute
        return Promise.resolve();
    }
    console.log(`Replacing ${kind} track for producer ${producer.id}`);
    return producer.replaceTrack({ track: newTrack })
