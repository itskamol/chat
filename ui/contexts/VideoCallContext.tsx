...
import {
    WebRTCClient,
    ProducerInfo as WebRTCProducerInfo,
    ConsumerInfo as WebRTCConsumerInfo,
} from '@/lib/webrtcClient'; // Adjust path
import {
    emitJoinRoom,
    emitLeaveRoom,
    onNewProducer,
...
export interface RemoteStreamInfo {
    // Renamed from RemoteStream to avoid confusion with MediaStream
...
const VideoCallContext = createContext<
    (VideoCallState & VideoCallActions) | undefined
>(undefined);
...
export const VideoCallProvider: React.FC<VideoCallProviderProps> = ({
    ...
    const [localStream, setLocalStreamState] = useState<MediaStream | null>(
        null
    );
    const [localAudioEnabled, setLocalAudioEnabled] = useState(false);
    const [localVideoEnabled, setLocalVideoEnabled] = useState(false);
    const [localScreenStream, setLocalScreenStreamState] =
        useState<MediaStream | null>(null);
    const [isScreenSharing, setIsScreenSharingState] = useState(false);
    const [screenShareProducerId, setScreenShareProducerIdState] = useState<
        string | null
    >(null);

    const [remoteConsumers, setRemoteConsumers] = useState<
        Map<string, WebRTCConsumerInfo>
    >(new Map());
    ...
    const setCallStatus = useCallback(
        (
            status: Partial<
                Pick<VideoCallState, 'isInCall' | 'isConnecting' | 'error'>
            >
        ) => {
        ...
        (participantId: string, updates: Partial<Participant>) => {
            setParticipantsState((prev) => {
                ...
                if (existingParticipant) {
                    return new Map(prev).set(participantId, {
                        ...existingParticipant,
                        ...updates,
                    });
                }
                if (
                    updates.id &&
                    (updates.name || updates.socketId || updates.isLocal)
                ) {
                    // Ensure enough info to create
                    return new Map(prev).set(
                        participantId,
                        updates as Participant
                    );
                }
                return prev;
            });
    ...
    const startLocalMedia = useCallback(
        async (audio = true, video = true): Promise<MediaStream | null> => {
            console.log(
                'Requesting local media with audio:',
                audio,
                'video:',
                video
            );
            try {
                ...
                setError(
                    (err as Error).message ||
                        'Failed to get user media. Check permissions and devices.'
                );
    ...
    const addParticipant = useCallback((participant: Participant) => {
        setParticipantsState((prev) =>
            new Map(prev).set(participant.id, {
                ...prev.get(participant.id),
                ...participant,
            })
        );
    }, []);
    ...
    const _addOrUpdateParticipantStream = useCallback(
        (
            participantId: string,
            ...
            setParticipantsState((prev) => {
                ...
                let targetStreamKey: 'stream' | 'screenStream' =
                    streamType === 'screen' ? 'screenStream' : 'stream';
                let newStream =
                    participant[targetStreamKey] || new MediaStream();

                // Ensure only one track of this kind in this specific stream
                const oldTracks = newStream
                    .getTracks()
                ...
                const updatedParticipant = {
                    ...participant,
                    [targetStreamKey]: newStream,
                };

                if (streamType === 'webcam') {
                    updatedParticipant.audioEnabled = newStream
                        .getAudioTracks()
                        .some((t) => t.enabled);
                    updatedParticipant.videoEnabled = newStream
                        .getVideoTracks()
                        .some((t) => t.enabled);
                } else {
                    // screen share
                    updatedParticipant.isScreenSharing = newStream
                        .getVideoTracks()
                        .some((t) => t.enabled); // Assuming screen share is video only
                }
    ...
    const _removeParticipantTrack = useCallback(
        (
            participantId: string,
            kind: 'audio' | 'video',
            streamType: 'webcam' | 'screen' = 'webcam'
            ...
            setParticipantsState((prev) => {
                ...
                if (streamType === 'webcam') {
                    updatedParticipant.audioEnabled = currentStream
                        .getAudioTracks()
                        .some((t) => t.enabled);
                    updatedParticipant.videoEnabled = currentStream
                        .getVideoTracks()
                        .some((t) => t.enabled);
                    if (!hasTracksLeft) updatedParticipant.stream = undefined;
                } else {
                    // screen share
                    updatedParticipant.isScreenSharing = currentStream
                        .getVideoTracks()
                        .some((t) => t.enabled);
                    if (!hasTracksLeft)
                        updatedParticipant.screenStream = undefined;
                }
    ...
    const initiateCall = useCallback(
        async (
            ...
            console.log(
                `Initiating call for room ${rId}, user ${currentLocalUserId}`
            );
            setRoomId(rId);
            ...
            if (!stream) {
                console.error(
                    'Failed to start local media, aborting call initiation.'
                );
                setIsConnecting(false);
                setError(
                    'Could not start microphone/camera. Please check permissions.'
                );
                setRoomId(null);
                setLocalUserId(null);
            ...
            updateParticipant(currentLocalUserId, {
                stream,
                audioEnabled: stream.getAudioTracks()[0]?.enabled,
                videoEnabled: stream.getVideoTracks()[0]?.enabled,
            });

            emitJoinRoom(rId, async (joinResponse) => {
                ...
                console.log(
                    'Successfully joined room. Active producers:',
                    joinResponse.activeProducers
                );
                setIsInCall(true);
                setIsConnecting(false);
                ...
                try {
                    if (!webrtcClientRef.current)
                        throw new Error('WebRTCClient not initialized');
                    ...
                    if (localAudioEnabled && audioTrack) {
                        try {
                            const audioProducer =
                                await webrtcClientRef.current.produce(
                                    audioTrack,
                                    {
                            ...
                            console.error('Error producing audio:', prodError);
                            setError(
                                'Error producing audio: ' +
                                    (prodError as Error).message
                            );
                        }
                    }
                    ...
                    if (localVideoEnabled && videoTrack) {
                        try {
                            ...
                            console.error('Error producing video:', prodError);
                            setError(
                                'Error producing video: ' +
                                    (prodError as Error).message
                            );
                        }
                    }
                    ...
                    console.error(
                        'WebRTC setup failed after joining room:',
                        webRtcErr
                    );
                    setError(
                        (webRtcErr as Error).message || 'WebRTC setup failed.'
                    );
                    emitLeaveRoom(rId);
                    stopLocalMedia();
                    setIsInCall(false);
    ...
    const toggleLocalAudio = useCallback(async () => {
        ...
        if (webrtcClientRef.current) {
            ...
            if (newEnabledState && !participant?.audioProducerId) {
                try {
                    const producer = await webrtcClientRef.current.produce(
                        audioTrack,
                        { userId: localUserId, type: 'webcam' }
                    );
                    updateParticipant(localUserId, {
                        audioProducerId: producer.id,
                    });
                } catch (e) {
                    console.error('Failed to produce audio on unmute:', e);
                    audioTrack.enabled = false;
                    setLocalAudioEnabled(false);
                    updateParticipant(localUserId, { audioEnabled: false });
                }
            } else if (!newEnabledState && participant?.audioProducerId) {
                webrtcClientRef.current.pauseProducer('audio');
            } else if (newEnabledState && participant?.audioProducerId) {
        ...
        if (webrtcClientRef.current) {
            ...
            if (newEnabledState && !participant?.videoProducerId) {
                try {
                    ...
                    console.error('Failed to produce video on unmute:', e);
                    videoTrack.enabled = false;
                    setLocalVideoEnabled(false);
                    updateParticipant(localUserId, { videoEnabled: false });
                }
    ...
    const startScreenShare = useCallback(async () => {
        if (
            ...
            !roomId ||
            !localUserId ||
            !webrtcClientRef.current?.getSendTransportId()
        ) {
            console.warn('Cannot start screen share:', {
                isScreenSharing,
                roomId,
                localUserId,
                transportExists:
        ...
        try {
            ...
            if (!transportId)
                throw new Error(
                    'Send transport not available for screen share'
                );
    ...
    const stopScreenShare = useCallback(async () => {
        ...
        console.log(
            'Stopping screen share, producerId:',
            screenShareProducerId
        );
        ...
        if (localUserId) {
            updateParticipant(localUserId, {
                screenShareProducerId: undefined,
    ...
    const _handleNewProducer = useCallback(
        async (payload: {
            producerId: string;
            userId: string;
            kind: 'audio' | 'video';
            ...
            if (
                ...
                !webrtcClientRef.current.isInitialized() ||
                !isInCall
            ) {
                console.warn(
                    'Received newProducer but WebRTC client not ready or not in call.',
                    payload
                );
                return;
            }
            if (payload.userId === localUserId) {
                console.log('Ignoring own newProducer event', payload);
                return;
            }
            ...
            if (!participants.has(userId)) {
                addParticipant({
                    id: userId,
                    name: appData?.name || `User ${userId.substring(0, 6)}`,
                    socketId,
                    audioEnabled: false,
                    videoEnabled: false,
                    isScreenSharing: false,
            ...
            if (remoteConsumers.has(producerId)) {
                console.warn(
                    `Already consuming producer ${producerId}. Ignoring.`
                );
                return;
            }
            ...
            try {
                const rtpCapabilities =
                    webrtcClientRef.current.getDeviceRtpCapabilities();
                if (!rtpCapabilities) {
                    console.error(
                        'Device RTP capabilities are not available. Cannot consume.'
                    );
                    setError(
                        'Device RTP capabilities are not available for consumption.'
                    );
                    return;
                }
                ...
                if (consumerInfo) {
                    setRemoteConsumers((prev) =>
                        new Map(prev).set(producerId, consumerInfo)
                    ); // Store consumer by producerId

                    if (producerType === 'screen') {
                        _addOrUpdateParticipantStream(
                            userId,
                            consumerInfo.track,
                            'screen'
                        );
                        ...
                        if (kind === 'audio')
                            updateParticipant(userId, { audioEnabled: true });
                        if (kind === 'video')
                            updateParticipant(userId, { videoEnabled: true });
                    }
                    console.log(
                        `Successfully consumed ${kind} (type: ${producerType}) from ${userId}, producer ${producerId}`
                    );
                }
            } catch (err) {
                console.error(
                    `Error consuming producer ${producerId} (${kind}, type: ${producerType}) from ${userId}:`,
                    err
                );
                setError(
                    `Failed to consume ${kind} from ${userId}: ${
                        (err as Error).message
                    }`
                );
            }
    ...
    const _handleProducerClosed = useCallback(
        (payload: { producerId: string; userId: string; socketId: string }) => {
            ...
            if (consumerInfo) {
                ...
                console.log(
                    `Consumer for producer ${producerId} (type: ${producerType}) closed and track removed for user ${userId}`
                );
            }
        },
    ...
    const _handleActiveProducers = useCallback(
        (
            producersToConsume: {
                producerId: string;
                kind: 'audio' | 'video';
    ...
    const updateParticipantMediaState = useCallback(
        (
            participantId: string,
            kind: 'audio' | 'video' | 'screen',
            enabled: boolean,
            ...
            if (producerId) {
                if (kind === 'audio')
                    updates.audioProducerId = enabled ? producerId : undefined;
                else if (kind === 'video')
                    updates.videoProducerId = enabled ? producerId : undefined;
                else if (kind === 'screen')
                    updates.screenShareProducerId = enabled
                        ? producerId
                        : undefined;
            }
    ...
    useEffect(() => {
        ...
        if (isInCall || isConnecting) {
            console.log(
                'Setting up signaling event listeners for VideoCallContext'
            );
        ...
        return () => {
            console.log(
                'Cleaning up signaling event listeners for VideoCallContext'
            );
    ...
    const _mockAddParticipant = useCallback(
        (p: Participant) => {
            ...
            const pWithStream = {
                ...
                stream,
                audioEnabled:
                    p.audioEnabled !== undefined ? p.audioEnabled : true,
                videoEnabled:
                    p.videoEnabled !== undefined ? p.videoEnabled : true,
    ...
    const _mockRemoveParticipant = useCallback(
        (participantId: string) => removeParticipant(participantId),
        [removeParticipant]
    );
    const _mockToggleParticipantVideo = useCallback(
        (participantId: string) => {
            updateParticipant(participantId, {
                videoEnabled: !participants.get(participantId)?.videoEnabled,
            });
        },
        [participants, updateParticipant]
    );
    const _mockToggleParticipantAudio = useCallback(
        (participantId: string) => {
            updateParticipant(participantId, {
                audioEnabled: !participants.get(participantId)?.audioEnabled,
            });
        },
        [participants, updateParticipant]
    );
    const _mockToggleScreenShare = useCallback(
        (participantId: string, isSharing: boolean) => {
            updateParticipant(participantId, {
                isScreenSharing: isSharing,
                screenStream: isSharing ? new MediaStream() : undefined,
            });
        },
        [updateParticipant]
    );
    ...
    useEffect(() => {
        return () => {
            ...
            if (isScreenSharing) {
                stopScreenShare();
            } // Ensure screen share is also stopped
            webrtcClientRef.current?.close();
        };
