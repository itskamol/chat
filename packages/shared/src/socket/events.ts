...
export interface ClientToServerEvents {
    ...
    [SocketEvent.GET_MESSAGES]: (
        payload: P.GetMessagesPayload,
        callback: (
            response: P.MessagesLoadedPayload | P.MessageErrorPayload
        ) => void
    ) => void;
    [SocketEvent.MARK_MESSAGE_AS_READ]: (
        payload: P.MarkMessageAsReadPayload,
        callback?: (response: P.MessageErrorPayload | P.EmptyPayload) => void
    ) => void;
    [SocketEvent.TYPING]: (payload: P.TypingPayload) => void;
    [SocketEvent.GET_ONLINE_USERS]: (
        callback: (response: P.OnlineUsersListPayload) => void
    ) => void;

    // WebRTC C2S (often with callbacks)
    [SocketEvent.JOIN_ROOM]: (
        payload: P.JoinRoomPayload,
        callback: (response: P.JoinRoomResponsePayload) => void
    ) => void;
    [SocketEvent.LEAVE_ROOM]: (
        payload: P.LeaveRoomPayload,
        callback: (response: P.LeaveRoomResponsePayload) => void
    ) => void;
    [SocketEvent.GET_ROUTER_RTP_CAPABILITIES]: (
        payload: P.GetRouterRtpCapabilitiesPayload,
        callback: (response: P.GetRouterRtpCapabilitiesResponsePayload) => void
    ) => void;
    [SocketEvent.CREATE_WEBRTC_TRANSPORT]: (
        payload: P.CreateWebRtcTransportPayload,
        callback: (response: P.CreateWebRtcTransportResponsePayload) => void
    ) => void;
    [SocketEvent.CONNECT_WEBRTC_TRANSPORT]: (
        payload: P.ConnectWebRtcTransportPayload,
        callback: (response: P.ConnectWebRtcTransportResponsePayload) => void
    ) => void;
    [SocketEvent.PRODUCE]: (
        payload: P.ProducePayload,
        callback: (response: P.ProduceResponsePayload) => void
    ) => void;
    [SocketEvent.CONSUME]: (
        payload: P.ConsumePayload,
        callback: (response: P.ConsumeResponsePayload) => void
    ) => void;
    [SocketEvent.START_SCREEN_SHARE]: (
        payload: P.StartScreenSharePayload,
        callback: (response: P.StartScreenShareResponsePayload) => void
    ) => void;
    [SocketEvent.STOP_SCREEN_SHARE]: (
        payload: P.StopScreenSharePayload,
        callback: (response: P.StopScreenShareResponsePayload) => void
    ) => void;
}
...
export interface ServerToClientEvents {
    ...
    [SocketEvent.ONLINE_USERS_LIST]: (
        payload: P.OnlineUsersListPayload
    ) => void;
    [SocketEvent.USER_STATUS_CHANGED]: (
        payload: P.UserStatusChangedPayload
    ) => void;
